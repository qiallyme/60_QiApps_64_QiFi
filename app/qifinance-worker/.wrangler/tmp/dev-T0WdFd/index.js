...
async function handleAssistant(request, env) {
  ...
  const resultMessage = summary || (createdCount > 0 ? `Created ${createdCount} record${createdCount === 1 ? "" : "s"}.` : "No records were created.");
  return json({
    ok: errorCount === 0,
...
async function createAssistantAccount(action, env, context) {
  ...
  const duplicate = context.accounts.find((account) => {
    ...
    const sameNumber = accountNumber && normalizeText(account.institution).toLowerCase() === institution.toLowerCase() && normalizeText(account.account_number ?? account.accountNumber) === accountNumber;
...
function accountExists(context, id) {
  return context.accounts.some((account) => account.id === id);
}
__name(accountExists, "accountExists");
...
function nextAccountCode(accounts, accountType) {
  const baseByType = {
    asset: 1e3,
    liability: 2e3,
    equity: 3e3,
  ...
  let next = max >= 0 ? max + 10 : baseByType[accountType] ?? 1e3;
...
__name(nextAccountCode, "nextAccountCode");
...
function runCategorizationHeuristics(rawDesc, rules) {
  ...
  if (cleanDesc.includes("dining") || cleanDesc.includes("mcdonald") || cleanDesc.includes("starbucks") || cleanDesc.includes("cafe")) {
    return { accountId: "expenses-dining", tags: ["dining", "meals"], counterparty: "Restaurant/Cafe", confidence: 0.8 };
  }
  if (cleanDesc.includes("lyft") || cleanDesc.includes("uber")) {
    return { accountId: "expenses-travel", tags: ["travel", "business"], counterparty: "Rideshare", confidence: 0.9 };
  }
  if (cleanDesc.includes("transfer") || cleanDesc.includes("payment") || cleanDesc.includes("venmo")) {
    return { accountId: "clearing-cc-payment", tags: ["transfer"], counterparty: "Cleared Fund Transfer", confidence: 0.7 };
  }
  return {
...
function mapAccountInput(body, isUpdate = false) {
  return compact({
    id: isUpdate ? void 0 : body.id,
    code: body.code,
    name: body.name,
    type: body.type,
...
__name(mapAccountInput, "mapAccountInput");
...
function mapTransactionInput(body, isUpdate = false) {
  ...
  return compact({
    ...
    import_status: body.importStatus ?? body.import_status ?? (isUpdate ? void 0 : "manual"),
    classification_status: body.classificationStatus ?? body.classification_status ?? (isUpdate ? void 0 : "classified"),
    ledger_status: body.ledgerStatus ?? body.ledger_status ?? (isUpdate ? void 0 : "not_posted"),
    source_metadata: body.sourceMetadata ?? body.source_metadata ?? {},
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  });
}
__name(mapTransactionInput, "mapTransactionInput");
async function replaceLedgerEntries(env, tx, categoryAccountId) {
  const txId = tx.id;
...
__name(replaceLedgerEntries, "replaceLedgerEntries");
