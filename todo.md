...
- `E21` Local account/subledger verification (2026-07-21): recursive chart flattening includes grandchildren and safely retains orphans/cycles; financial-account and ledger-account cards deep-link to filtered transaction ledgers; expanded transactions support shared-form editing and inline statement assignment. TransactionForm supports partial/full counterparty allocations with distinct shared, gift, and IOU treatments. A server-only, RLS-protected allocation table and atomic replacement function create obligations only for IOUs, so gifts/shared allocations remain tracking-only and no duplicate cash transaction is created. QiFi TypeScript, 10 tests, lint, and production build passed; QiApi TypeScript, ESLint, 50 tests, and Worker dry-run passed. Migration, deploy, and authenticated production mutation proof remain required.
...
- [ ] Apply the transaction-allocation migration, deploy QiApi/Pages, and verify create/edit/replacement atomically with an authenticated prefixed production fixture and cleanup.
- [ ] Verify counterparty ledgers show allocated gifts/shared spend while only IOU allocations increase active debt.
