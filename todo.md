...
- `E20` QiApi commit `d99f957` adds the server-authoritative Cash App adapter and fail-closed import staging; Worker version `76912398-4d5e-4543-aa24-c3938c7ff60d` is deployed. The adapter separates purchases, P2P, deposits, withdrawals, savings transfers, debt movements, refunds, and unknowns; preserves reference/subtotal/fee metadata; uses exact provider references for duplicate checks; and routes low-confidence, suspense, duplicate, transfer, P2P, and debt rows away from automatic posting. The real export produced 3,085 candidates with 2,407 conservatively requiring review. QiApi TypeScript, ESLint, 46 tests, and Cloudflare dry-run passed; QiFi’s 8 tests, TypeScript, and production build passed. The frontend now requires a Worker preview and explicit count-based confirmation before staged commit. Production browser run `29808510105` passed 48 authenticated route/viewport checks including Import and Review Queue plus PWA control. No production financial rows were imported.
- `E21` Account/subledger verification (2026-07-21): recursive chart flattening includes grandchildren and safely retains orphans/cycles; financial-account and ledger-account cards deep-link to filtered transaction ledgers; expanded transactions support shared-form editing and inline statement assignment. TransactionForm supports partial/full counterparty allocations with distinct shared, gift, and IOU treatments. Supabase migrations `add_transaction_allocations` and `fix_transaction_allocation_replacement` are applied; the server-only RLS table and atomic replacement function create obligations only for IOUs, so gifts/shared allocations remain tracking-only and no duplicate cash transaction is created. QiApi `980c225` deployed as Worker version `a2190f10-cad2-48f2-8d48-70fb21146a24`. QiFi TypeScript, 10 tests, lint, and production build passed; QiApi TypeScript, ESLint, 50 tests, and Worker dry-run passed. Authenticated production smoke run `29814761222` verified isolated create, IOU/gift allocation, atomic replacement with shared allocation, stale-IOU removal, and cleanup. Authenticated browser run `29817751733` passed the production route/viewport/PWA audit after the coordinated schema/API deployment.
- `E22` Bank-free opening debt verification (2026-07-21): Accountability now offers an explicit old-debt/opening-balance option only for “I Owe” entries, requires a liability ledger account, and never requests a financial/bank account. The atomic `create_opening_debt` database function creates the obligation and posts debit Opening Balance Equity / credit selected liability with no transaction or expense row; journal-backed obligation deletion also removes its journal and lines. Migrations are applied and QiApi `9c65bc5` is deployed as Worker version `59808dbb-1b4e-4dc9-abb0-4e567ad47c53`. QiFi TypeScript, 10 tests, and production build passed; QiApi TypeScript, ESLint, 50 tests, and Worker dry-run passed. Authenticated production smoke run `29903506739` verified the exact journal lines, absence of a transaction, and cleanup. Browser/PWA audit `29903757770` passed after the coordinated release.
- `E23` Batch receipt inbox implementation (2026-07-22): QiFi now has a durable Receipt Inbox backed by unattached existing `attachments`, multi-file upload plus mobile camera capture, batch/retry OCR processing states, amount/date/merchant transaction-match ranking with ambiguous-match blocking, missing counterparty/category warnings, and shared `TransactionForm` draft review. Approval either associates the receipt with a user-selected existing transaction or creates a reviewed transaction and then associates it; OCR never silently finalizes a transaction. QiApi adds an authenticated sparse attachment PATCH route, with a regression test proving approval cannot overwrite stored file metadata. QiFi TypeScript, 13 tests, and production build passed; QiApi TypeScript, ESLint, and 51 tests passed. QiApi `7df3e97` is deployed as Worker version `7ae5fee2-a93f-4c6c-9ab9-83c04f30a5ee`, and production Pages serves the `ReceiptInboxView` bundle from QiFi `9300b04`. Authenticated production browser/PWA audit run `29913021049` passed across the corrected routes including `/receipts/inbox`; batch OCR mutation verification remains open.
...
- [x] Make the UI call the Worker preview endpoint and display duplicate, exclusion, sign, and mapping results before any commit. Evidence: E20.
- [x] Prevent `/import/commit` from silently finalizing unreviewed or suspense-mapped transactions; route ambiguous rows and transfers through review. Evidence: E20.
...
- [x] Add a durable batch Receipt Inbox with camera/multi-file intake, batch processing, retry/delete, explicit shared-form approval, duplicate matching, and missing-list warnings. Evidence: E23.
- [x] Keep receipt approval on the existing attachment model and update only the approved transaction association without overwriting storage/OCR metadata. Evidence: E23.
- [ ] Deploy the batch Receipt Inbox and attachment PATCH route, then verify multi-file upload, OCR, retry, match, new-draft approval, missing-list handling, and cleanup with the authenticated production smoke user.
...
- [x] Render every level of the chart-of-accounts hierarchy, including grandchildren, with orphan/cycle safety. Evidence: E21.
- [x] Deep-link financial and ledger accounts to an account-filtered transaction ledger with shared-form editing and inline reconciliation selection. Evidence: E21.
- [x] Implement partial/full counterparty allocation UI and server contracts for shared tracking, gifts, and IOUs without duplicate cash transactions. Evidence: E21.
- [x] Apply the transaction-allocation migrations, deploy QiApi/Pages, and verify create/edit/replacement atomically with an authenticated prefixed production fixture and cleanup. Evidence: E21.
- [x] Verify the allocation contract keeps gifts/shared spend out of debt while only IOUs create active obligations; the counterparty UI renders all allocation treatments. Evidence: E21.
- [x] Add a bank-free opening-debt workflow that posts liability against Opening Balance Equity without creating an expense or cash transaction. Evidence: E22.
